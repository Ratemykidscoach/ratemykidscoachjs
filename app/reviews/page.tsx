...
function makeReviewKey(r: ReviewRow) {
  return r.review_id; // ✅ use the real PK
}
...
function starsString(rating: number | null) {
  ...
  const full = Math.round(rating); // match your HTML style (whole stars)
  let s = '';
...
export default function ThankYouReviewPage() {
  ...
  function closeModal() {
    ...
    router.replace(qs ? `/thankyou_review?${qs}` : '/thankyou_review');
  }
  ...
  return (
    <>
      <div className="page">
        {/* Modal */}
        <div className={`modal-overlay ${showModal ? '' : 'hidden'}`} id="thankYouModal" onClick={(e) => {
          ...
          <div className="modal">
            ...
            <div className="modal-content">
              <h2>Thank You!</h2>
              <p>
                Your review has been successfully submitted. We appreciate you taking the time to help other parents make informed decisions about their children's coaches.
              </p>
        ...
        <div className="reviews-container">
          ...
          {!loading && error && (
            <div style={{ textAlign: 'center', color: '#8b2e2e' }}>
              {error}
            </div>
          )}
          ...
          {!loading && !error && computed.length === 0 && (
            <div style={{ textAlign: 'center', color: '#4a4a4a' }}>
              No reviews yet. Be the first to <Link href="/review_form" style={{ color: '#A8BCA1', textDecoration: 'none', fontWeight: 600 }}>leave a review</Link>.
            </div>
          )}
            ...
            computed.map((r) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`coaching-status ${timing.current ? 'current' : ''}`}>
                    {timing.text}
                  </div>

                  <div className="review-text">
                    {r.overall_review || 'No written review provided.'}
                  </div>
                </div>
              );
            })}
      ...
      {/* Global CSS copied from your HTML (kept nearly identical) */}
      <style jsx global>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .review-card:hover { box-shadow: 0 4px 8px rgba(0,0,0,0.1); }
        ...
        .club-name { color: #4a4a4a; font-size: 0.9em; margin-bottom: 8px; }

        .review-meta { color: #718096; font-size: 0.85em; }
        ...
        .review-card.expanded .coaching-status { display: block; }
        ...
        .vote-btn:hover { background: #EDE6DB; border-color: #B79973; }
        ...
        .vote-icon svg { width: 16px; height: 16px; fill: currentColor; }
        ...
        .stars { color: #B79973; font-size: 1.1em; letter-spacing: 1px; }

        .rating-number { font-weight: 600; color: #1a1a1a; font-size: 0.95em; }
        ...
        .review-card.expanded .review-categories { display: grid; }

        .category { font-size: 0.85em; }

        .category-name { color: #4a4a4a; margin-bottom: 3px; font-weight: 500; }

        .category-rating { display: flex; align-items: center; gap: 5px; }

        .mini-stars { color: #B79973; font-size: 0.9em; }
        ...
        .review-card.expanded .review-text { display: block; }
        ...
        .review-card.expanded .expand-indicator { display: none; }
        ...
        .close-btn:hover { background: #f7fafc; color: #1a1a1a; }
        ...
        .modal p { color: #4a4a4a; line-height: 1.6; margin-bottom: 30px; }
        ...
        .btn-primary { background: #A8BCA1; color: white; }
        .btn-primary:hover { background: #92a786; }

        .btn-secondary { background: #F9F7F4; color: #1a1a1a; }
        .btn-secondary:hover { background: #EDE6DB; }
